...
// Ayah search is Arabic-only. A non-empty query is treated as searchable only
// when it contains Arabic letters; Latin-script (transliteration) matching was
// removed so users search by the Qur'anic text itself.
...
export function searchCached(query: string): SearchHit[] {
  // Ayah search is Arabic-only: a query with no Arabic letters can't match an
  // ayah (transliteration matching was removed), so bail early.
  if (!ARABIC_LETTER_RE.test(query)) return [];
